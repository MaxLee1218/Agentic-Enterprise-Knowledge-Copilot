...
def test_database_failure_keeps_process_live_but_fails_readiness() -> None:
    application = create_app(
        readiness=ReadinessService(
            {"database": lambda: False, "artifact_storage": lambda: True},
            task_dependencies=frozenset({"database", "artifact_storage"}),
        )
    )
    ...
    assert payload == {
        "status": "not_ready",
        "accepts_tasks": False,
        "dependencies": {"database": "unavailable", "artifact_storage": "ok"},
    }
